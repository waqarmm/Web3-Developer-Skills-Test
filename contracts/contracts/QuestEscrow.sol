...
/**
 * @title QuestEscrow
 * @dev On-chain bounty escrow. Posters lock ETH or ERC20 tokens,
 *      workers complete quests, and the contract handles payouts
 *      with a 3% platform fee.
 */
contract QuestEscrow is ReentrancyGuard, Ownable {
    ...
    event QuestRefunded(uint256 indexed questId, address indexed poster);
    ...
    function cancelQuest(uint256 questId) external nonReentrant {
        ...
        _sendFunds(q.token, q.poster, q.reward);
    ...
    /// @dev Calculate fee, pay the worker, mark quest completed.
    function _completePayout(uint256 questId) internal {
        ...
        _sendFunds(q.token, q.worker, payout);

        emit QuestCompleted(questId, q.worker, payout);
    }

    /// @dev Transfer ETH or ERC20 to a recipient.
